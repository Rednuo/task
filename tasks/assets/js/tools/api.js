(function () {
    ...
    (function (u) {
        ...
        for (var t = EncJS, r = t.lib, w = r.WordArray, v = r.Hasher, r = t.algo, b = [],
    ...
    (function () {
        var u = EncJS, p = u.lib, d = p.Base, l = p.WordArray, p = u.algo, s = p.EvpKDF = d.extend({
            cfg: d.extend({
                keySize: 4,
                hasher: p.MD5,
                iterations: 1
    ...
    EncJS.lib.Cipher || function (u) {
        var p = EncJS, d = p.lib, l = d.Base, s = d.WordArray, t = d.BufferedBlockAlgorithm, r = p.enc.Base64,
    ...
    (function () {
        for (var u = EncJS, p = u.lib.BlockCipher, d = u.algo, l = [], s = [], t = [], r = [], w = [], v = [], b = [],
        ...
        var H = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54], d = d.AES = p.extend({
            _doReset: function () {
                ...
                for (var m = this._nRounds, g = a[b] ^ c[0], h = a[b + 1] ^ c[1], k = a[b + 2] ^ c[2],
                         n = a[b + 3] ^ c[3], p = 4,
    ...
    (function () {
        ...
        function n(p) {
            e.lastIndex = 0;
            return e.test(p) ? "\"" + p.replace(e, function (q) {
                var r = l[q];
                return typeof r === "string" ? c : "\\u" + ("0000" + q.charCodeAt(0).toString(16)).slice(-4)
            }) + "\"" : "\"" + p + "\""
        }
        ...
        if (typeof JSON.parse !== "function") {
            JSON.parse = function (p, q) {
                ...
                if (a.test(p.replace(b, "@").replace(c, "]").replace(d, ""))) {
                    r = eval("(" + p + ")");
                    return (typeof q === "function") ? s({"": r}, "") : r
                }
...
// 加解密配置
    var apiHelper = {
        ...
        getAesString: function (data) {
            return EncJS.AES.encrypt(data, this.key,
                this);
        },
        getDAesString: function (data) {
            return EncJS.AES.decrypt(data, this.key,
                this).toString(EncJS.enc.Utf8);
        }
    };
...
// API工具对象
    window.ApiUtil = {
        format: function (json) {
            return encodeURIComponent(apiHelper.getAesString(JSON.stringify(json)));
        },
        unformat: function (data) {
            return apiHelper.getDAesString(data);
        },
        domain: 'https://mhjkm.1391.com',
        payDomain: 'https://mhdpay.1391.com',
        param: '?apptype=13&appversion=1.0&channel=web-app',
        payParam: '?apptype=13&appversion=1.0&channel=web-app'
    };
})();
